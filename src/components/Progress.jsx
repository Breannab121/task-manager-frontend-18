import React from "react";

// Progress bar component that visually represents task progress with color based on status
const Progress = ({ progress, status }) => {
  
  // Determines the color classes based on the task status
  const getColor = () => {
    switch (status) {
      case 'In Progress':
        return 'text-cyan-500 bg-cyan-500 border border-cyan-500/10';

      case 'Completed':
        return 'text-indigo-500 bg-indigo-500 border border-indigo-500/10';

      default:
        return 'text-violet-500 bg-violet-500 border border-violet-500/10';
    }
  };

  return (
    // Outer gray progress track
    <div className="w-full bg-gray-200 rounded-full h-1.5">
      {/* Inner colored progress fill based on status and dynamic width */}
      <div
        className={`${getColor()} h-1.5 rounded-full text-center text-xs font-medium`}
        style={{ width: `${progress}%` }}
      >
      </div>
    </div>
  );
};

export default Progress;
